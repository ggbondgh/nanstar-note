...
export async function onRequestDelete({ env, request }) {
  logRequest("files:delete", request);
  const auth = authorize(env, request);
  if (auth) return auth;
...
function corsHeaders() {
  return {
    ...
    "access-control-allow-headers": "content-type, authorization, x-client-id, x-client-session-id, x-client-source, x-client-page, x-client-purpose, x-file-name, x-file-size"
  };
}
