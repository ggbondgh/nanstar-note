...
The file-transfer assistant is separate from TXT/MD/DOC notes. It presents text, images, and temporary files in one chronological message stream. Text can be pasted into the composer, while images and files can also be pasted or dragged into the message area.
...
File transfer stores file bytes in Cloudflare R2 and stores only file metadata in D1. Current limits are 8 files, 35 MB per file, and 512 MB total.
